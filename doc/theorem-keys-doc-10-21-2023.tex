...
\tcbset{preamble/.style = { before upper = {\tcbusetemplisting\texttt{\dots}}}}
...
\begin{docKey}{restate-counters}{=\marg{comma-list of counters}}{initially \ttbrackets{equation}}
The counters whose values are preserved when a theorem is restated.
\end{docKey}

\begin{docKey}{continues-code}{=\meta{code with \textup{\texttt{\#1}}}}{initially |continuing from p.\cs{,}\cs{pageref}\ttbrackets{\#1}|}
...
This is \pkg{theorem-keys}' version of \pkg{thmtools}' \cs{declaretheoremstyle}\oarg{options}\marg{name}. Since it makes little sense to define a style with no keys, we've made the \meta{options} argument mandatory.
\end{docCommand}
...
\begin{docKey}{headpunct}{=\meta{code}}{initially \ttbrackets{.}}
...
\begin{docKey}{notebraces}{=\marg{left brace}\marg{right brace}}{initially \ttbrackets{(}\ttbrackets{)}}
...
\begin{docKey}{restate}{=\meta{csname}}{initially unset}
Defines a command \texttt{\char`\\}\meta{csname} that can be used to restate the theorem, including the body text, later in the document.
\end{docKey}
...
\begin{docKey}{title-code}{=\meta{code with \textup{\texttt{\#1}}}}{initially \cs{section*}\ttbrackets{\#1}}
If \cs{chapter} is defined, then initially this is instead \cs{chapter*}\ttbrackets{\#1}.
\end{docKey}
