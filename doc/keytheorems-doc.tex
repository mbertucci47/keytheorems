...
\begin{docKey}{thmtools-compat}
  {}
  {initially unset}
For compatibility with \pkg{thmtools} syntax. Currently defines the following commands:
\begin{center}
\begin{tabular}{cc}
...
This is \pkg{keytheorems}' version of \pkg{thmtools}' \cs{declaretheoremstyle}\bracks{\meta{options}}\marg{name}. Since it makes little sense to define a style with no keys, we've made the \meta{options} argument mandatory.
\end{docCommand}

\begin{notebox}
For the AMS classes \cls{amsart}, \cls{amsbook}, and \cls{amsproc}, the initial key values are slightly different than what's below in order to match those class's defaults.
\end{notebox}
...
Defaults to ``List of Theorems'' if English or an unknown language is used. Currently French, German, and Spanish have (probably bad!) translations. A translation can be added with a Github pull request or manually with \cs{DeclareTranslation}\marg{lang}\ttbraces{keythms\string_listof\string_title}\marg{text}.
\end{docKey}
