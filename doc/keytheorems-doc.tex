...
\begin{docKey}{auto-qed}
  {\colOpt{=unnumbered\textbar all\textbar none}}
  {default none, initially |unnumbered|}
Alias \docAuxKey{thmmarks}. % maybe??
Controls the automatic qed placement feature; see Section \ref{auto-qed}.
The default option, |unnumbered| enables automatic qed placement only when the final inner environment has no equation number.
Option |all| enables the feature for all (supported) environments, and |none| disables the feature.

The default is |unnumbered| because qed marks are placed at right and therefore can conflict with right-justified equation numbers.
\begin{notebox}
NOTE: TODO this behavior is not yet implemented
\end{notebox}
If \pkg{keytheorems} detects that equation numbers are left-justified, then the default option changes to |all|.
This occurs with the AMS document classes such as |amsart| and with the document option |leqno| for classes that support it.
A user-supplied option for |auto-qed| always overrides these default behaviors.

See subsection \ref{auto-qed-option-environments} for a complete list of final environments supported by the |unnumbered| and |all| options.  
\end{docKey}
...
The package option \refKey{auto-qed} controls this feature, with options |unnumbered|, |all|, and |none|.
Subsection \ref{auto-qed-option-environments} documents which closing environments are included by each option.

This is accomplished via the \hook{env/<env>/end} hook.
Subsection \ref{auto-qed-implementation} documents further implementation details.
