...
\tcbuselibrary{documentation,minted,hooks}
\tcbset{
...
\colorlet{ExampleBack}{yellow!10!white}
\tcbset{
...
The package tries to adapt to \cls{beamer}'s changes to internal \pkg{amsthm} commands, however the same restrictions on theorems defined with \cs{newtheorem} apply here.
Most style keys are disabled by the |default| \cls{beamer} theorem template.
More become available by setting
\begin{dispListing}
...
\pkg{keytheorems} does not yet support the optional \texttt{<\meta{overlay specification}>} argument for theorems.
