...
\begin{docKey}{predefined}
  {\colOpt{=\marg{options}}}
  {initially unset}
This is a convenience key, similar to \pkg{ntheorem}'s |standard| option, that predefines a set of theorems that, unless \refKey{auto-translate} is set to |false|, are translated into the current language if translations exist. The predefined theorems are
\begin{itemize}
...
These theorems are provided at the end of the preamble (specifically, in the \hook{begindocument} hook) with \refCom{providekeytheorem} so will not overwrite user-defined environments with the same name. By default, the predefined theorems share a counter and do not have a parent counter. These settings can be changed by calling |siblings=false| (alias |sharenumbers=false|) and/or |parent=|\meta{counter}, respectively, in \meta{options}.
...
If no value is given, current value of |\qedsymbol| is used (one can redefine this or set it with \refKey{qed-symbol}).
...
Both \pkg{cleveref} and \pkg{zref-clever} define default reference names for some commonly used counters like |theorem|, |lemma|, etc. For technical reasons, unless explicit values for \refKey{refname} and \refKey{Refname} are given, \pkg{keytheorems} does not try to change these defaults at all in the case of \pkg{cleveref} and only the singular name in the case of \pkg{zref-clever}. The easiest way to get exactly the output you want is to just explicitly set \refKey{refname} and \refKey{Refname}.
...
\begin{notebox}
For the AMS classes \cls{amsart}, \cls{amsbook}, and \cls{amsproc}, as well as the \cls{amsart}-based \cls{acmart} and \cls{aomart}, the initial key values are slightly different those listed in \zcref{stylekeys:new,stylekeys:thmtools} in order to match those class's defaults. See \autoref{class-support} for details.
\end{notebox}
...
The package contains some \emph{highly experimental} code to support theorems with \cls{beamer}, including overlays. The support differs based on whether or not the \cls{beamer} class option |noamsthm| is used.
...
If |noamsthm| is loaded, you can use keytheorems exactly as you would in any other document except that \refCom{listofkeytheorems} is disabled. This includes no limitations on storing and restating theorems that exist without |noamsthm|.
...
Lastly, in addition to the \cls{beamer} support outlined in \zcref{subsec:beamer}, support is provided for overlays in the experimental tagging-compatible presentation class \cls{ltx-talk}. See \zcref{subsec:tagging} for more details.
...
As mentioned above, support is also provided for the tagging-compatible \cls{ltx-talk} presentation class. You may use \pkg{keytheorems} exactly as you would in a normal tagged document, with the added support of overlays. The syntax for overlays is the same as for \cls{beamer}, shown in \zcref{subsubsec:noamsthm}. Also as for \cls{beamer}, the \refCom{listofkeytheorems} command is disabled.
