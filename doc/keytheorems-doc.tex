...
\end{itemize}
These keys patch the \cs{@thm} command with \pkg{etoolbox}'s \cs{patchcmd} hence can break with changes to \cs{@thm}.
This is accounted for with the changes made by \pkg{cleveref} and \pkg{theoremref} but other packages might break them.
\begin{tcbwritetemp}
