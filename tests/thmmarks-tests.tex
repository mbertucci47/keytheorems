...
Note: The endmark is missing from the previous example, but not the next one.
This seems like a bug.

\makeoutertest{split}{
  This \texttt{split} is inside \texttt{gather}.
}{
  \begin{gather}
    % a + b = c\\
    \begin{split}
...
\begin{test}
  \texttt{tabular}: 
  a table in a \texttt{center}, with \verb|\qedabove| to manually position the endmark
  \begin{center}
    \begin{tabular}{l|l}
      \textit{Player name}  &\textit{Career home runs}  \\
      \hline
