...
Note: the unnumbered variant, \texttt{alignat*}, is currently unsupported.
...
\maketest{quote}{}{
  ...
  The \texttt{article} document class handles paragraphing in \texttt{quote} and \texttt{quotation} slightly differently.
  They are the same in the \texttt{amsart} document class.
}
...
\makeoutertest{alignat*}{
  unnumbered variant of \texttt{alignat}\\
  Currently unsupported.
  Our methods for endmark placement of other environments do not work for \texttt{alignat*}.
  We have not yet found an alternative.
}{
  \begin{alignat*}{2}
    x& = y_1-y_2+y_3-y_5+y_8-\dots
    &\qquad& \text{by (C)}\\
